...
class MainApp(App):
    ...
    def reset_all(self):
        ...
        self.opp= ""
        self.you= ""
        self.root.ids.play.ids.opponent.source= 'all.png'
        self.root.ids.play.ids.you.source= 'all.png'
        self.root.ids.play.ids.score.text= 'score'
    ...
    def process(self, me, him):
        ...
        for i in range(3):
            if self.me == 'scissors' and self.him == 'scissors':
                self.res= 'tie'
            if self.me == 'scissors' and self.him == 'paper':
                self.res= 'win'
            if self.me == 'scissors' and self.him == 'rock':
                self.res= 'foe'
            if self.me == 'paper' and self.him == 'scissors':
                self.res= 'foe'
            if self.me == 'paper' and self.him == 'paper':
                self.res= 'tie'
            if self.me == 'paper' and self.him == 'rock':
                self.res= 'win'
            if self.me == 'rock' and self.him =='scissors':
                self.res= 'win'
            if self.me == 'rock' and self.him == 'paper':
                self.res= 'foe'
            if self.me == 'rock' and self.him == 'rock':
                self.res= 'tie'
        return self.res
    ...
    def change_img(self, *args):
        ...
        if self.clocks <= 20:
            ...
            self.one= {1:'rocke.png', 2:'pepper.png', 3:'sis.png'}[randint(1,3)]
            opponent.source= self.one
        else:
            self.bomb.cancel()
            if self.one == 'rocke.png':
                self.opp= 'rock'
            elif self.one == 'pepper.png':
                self.opp= 'paper'
            elif self.one == 'sis.png':
                self.opp= 'scissors'
    ...
    def show_popup(self, why_popup):
        if why_popup == 'BtnUnclicked':
            pop= Popups(title='little warning!',
                        txt='you should select one of the three options first...')
        elif why_popup == 'exit':
